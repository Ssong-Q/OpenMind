...
function AccountForm ({name, image, id}) {
  ...
  return(
    <Style.Form>
      <InputImage name="imgFile" value={values.imgFile} onChange={handleChange} />
      <Style.Container>
        <Style.Input name='name' value={values.name} onChange={handleUserNameChange} />
        <Style.Button>이름 변경</Style.Button>
        <Style.Button onClick={handleDeleteClick}>계정 삭제</Style.Button>
      </Style.Container>
      <AccountDelete onClick={handleDeleteAccountClick} visible={visible} setVisible={setVisible} />
    </Style.Form>
  )
}
